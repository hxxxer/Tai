...
## 开发者：自动构建与发布（GitHub Actions）

> 面向仓库维护者的最小流程：日常提交自动构建，发布时通过 tag 自动打包并创建 Release。

- `CI Build`：当代码推送到 `main` 或发起 PR 到 `main` 时自动编译，并上传临时构建产物；
- `Release from Tag`：当推送 `vX.Y.Z` 格式的 tag（例如 `v1.2.3`）时，自动构建并发布 Release 附件。

### 发布步骤（维护者）

1. 确保 `main` 分支上的 CI 已通过；
2. 在本地创建并推送版本 tag：
...
3. 等待 GitHub Actions 完成，随后在 Releases 页面下载自动生成的压缩包。
